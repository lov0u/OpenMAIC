...
/** Common headers for all AGNES-AI Video API calls */
...
export async function testAgnesVideoConnectivity(
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      ...
      return {
        success: false,
        message: `AGNES-AI Video auth failed (${response.status}): ${text}`,
      };
    }
    return { success: true, message: 'Connected to AGNES-AI Video' };
  } catch (err) {
    return { success: false, message: `AGNES-AI Video connectivity error: ${err}` };
  }
}
...
async function submitVideoGeneration(
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`AGNES-AI video submit failed (${response.status}): ${text}`);
  }

  const data = (await response.json()) as AgnesVideoSubmitResponse;
  if (!data.request_id) {
    throw new Error('AGNES-AI video returned empty request_id');
  }
...
async function pollVideoStatus(
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`AGNES-AI video poll failed (${response.status}): ${text}`);
  }
...
export async function generateWithAgnesVideo(
  ...
  for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
    ...
    if (result.status === 'done') {
      if (!result.video?.url) {
        throw new Error('AGNES-AI video task completed but no video URL returned');
      }
    ...
    if (result.status === 'failed') {
      throw new Error(`AGNES-AI video generation failed: ${JSON.stringify(result)}`);
    }
  }

  throw new Error(
    `AGNES-AI video generation timed out after ${(MAX_POLL_ATTEMPTS * POLL_INTERVAL_MS) / 1000}s (request: ${requestId})`,
  );
}
